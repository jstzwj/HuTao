...
namespace hutao
{
    ...
    inline std::uint32_t HuStringChars::utf8_first_byte(std::uint8_t b, std::uint32_t width)
    {
        return b & (0x7F >> width);
    }

    inline std::uint32_t HuStringChars::utf8_acc_cont_byte(std::uint32_t ch, std::uint8_t b)
    {
        return (ch << 6) | (b & CONT_MASK);
    }

    inline std::uint8_t HuStringChars::get_byte_or_0(const std::uint8_t*& i, const std::uint8_t* end)
    {
        if (i < end) return *(i++);
        else return 0;
    }
    ...
    inline std::size_t HuChar::len_utf8(std::uint32_t code)
    {
        if (code < MAX_ONE_B)
            return 1;
    ...
    inline std::size_t HuChar::len_utf16(std::uint16_t code)
    {
    ...
    inline std::size_t HuChar::encode_utf8_raw(CodePointType code, CharType* dst, std::size_t dst_len)
    {
    ...
    inline std::size_t HuChar::encode_utf16_raw(CodePointType code, CharType* dst, std::size_t dst_len)
    {
